...
class headPoseEstimation(Resource):
    ...
    def post(self):
        ...
        headposeJson = Head_Pose().getHeadPose(frame).get_json()
        if headposeJson['status'] == 1:
            if headposeJson['Y'] > 30:
                headpose = "bad"
                headmessage = "不要向左看"
                return jsonify({"status": 0, "headstatus": headpose, "headmessage": headmessage})
            elif headposeJson['Y'] < -30:
                headpose = "bad"
                headmessage = "不要向右看"
                return jsonify({"status": 0, "headstatus": headpose, "headmessage": headmessage})
        return jsonify({"status": 1, "headstatus": "OK", "code": "OK"})


logging.basicConfig(level=logging.INFO)
api.add_resource(headPoseEstimation, '/face/head')
